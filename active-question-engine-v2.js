(() => {
  ...
  load('VERIFIED_Q_V3_EXTRA','data/verified-question-bank-v3-extra60.js','6000');
  load('VERIFIED_Q_V4_EXTRA','data/verified-question-bank-v4-extra100.js','6000');
  load('VERIFIED_Q_V5_EXTRA','data/verified-question-bank-v5-extra300.js','6000');
  load('VERIFIED_Q_V6_A','data/verified-question-bank-v6-a100.js','6000');
  load('VERIFIED_Q_V6_B','data/verified-question-bank-v6-b100.js','6000');
  load('VERIFIED_Q_V6_C','data/verified-question-bank-v6-c100.js','6000');
  load('VERIFIED_Q_V6_D','data/verified-question-bank-v6-d100.js','6000');
  for(let v=10;v<=19;v++) load(`VERIFIED_Q_V${v}_EXTRA`,`data/verified-question-bank-v${v}-extra100.js`,'6000');
  load('VERIFIED_Q_V20_EXTRA','data/verified-question-bank-v20-extra2000.js','6000');
  load('VERIFIED_Q_V21_EXTRA','data/verified-question-bank-v21-extra2000.js','6000');
  const pools=[
  ...
  const expected=[90,50,60,100,300,100,100,100,100,100,100,100,100,100,100,100,100,100,100,2000,2000];
  ...
  if(sizeBad||combined.length!==6000||bad.length||dup.length){throw new Error(`Aktif soru havuzu QA başarısız: sizes=${sizes.join(',')}, total=${combined.length}, bad=${bad.map(q=>q.sourceId).join(',')}, dup=${dup.join(',')}`)}
  ...
  window.ARENA_ACTIVE_BANK_META={count:6000,optionCount:5,batches:sizes,status:'active'};console.info('9. Sınıf Arena aktif soru motoru:',window.ARENA_ACTIVE_BANK_META);
})();