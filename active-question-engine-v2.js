(() => {
  ...
  load('VERIFIED_Q_V3_EXTRA','data/verified-question-bank-v3-extra60.js','2000');
  load('VERIFIED_Q_V4_EXTRA','data/verified-question-bank-v4-extra100.js','2000');
  load('VERIFIED_Q_V5_EXTRA','data/verified-question-bank-v5-extra300.js','2000');
  load('VERIFIED_Q_V6_A','data/verified-question-bank-v6-a100.js','2000');
  load('VERIFIED_Q_V6_B','data/verified-question-bank-v6-b100.js','2000');
  load('VERIFIED_Q_V6_C','data/verified-question-bank-v6-c100.js','2000');
  load('VERIFIED_Q_V6_D','data/verified-question-bank-v6-d100.js','2000');
  for(let v=10;v<=19;v++) load(`VERIFIED_Q_V${v}_EXTRA`,`data/verified-question-bank-v${v}-extra100.js`,'2000');
  const pools=[
  ...
  const expected=[90,50,60,100,300,100,100,100,100,100,100,100,100,100,100,100,100,100,100];
  const sizes=pools.map(p=>p.length);
  const combined=pools.flat().map((q,i)=>({...q,sourceId:q.id,id:i}));
  const bad=combined.filter(q=>!Array.isArray(q.o)||q.o.length!==5||new Set(q.o).size!==5||!Number.isInteger(q.a)||q.a<0||q.a>4||!q.q||!q.subject);
  const ids=combined.map(q=>q.sourceId),dup=ids.filter((id,i)=>ids.indexOf(id)!==i);
  const sizeBad=sizes.some((n,i)=>n!==expected[i]);
  if(sizeBad||combined.length!==2000||bad.length||dup.length){throw new Error(`Aktif soru havuzu QA başarısız: sizes=${sizes.join(',')}, total=${combined.length}, bad=${bad.map(q=>q.sourceId).join(',')}, dup=${dup.join(',')}`)}
  ...
  window.ARENA_ACTIVE_BANK_META={count:2000,optionCount:5,batches:sizes,status:'active'};console.info('9. Sınıf Arena aktif soru motoru:',window.ARENA_ACTIVE_BANK_META);
})();