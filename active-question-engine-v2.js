(() => {
  ...
  load('VERIFIED_Q_V3_EXTRA','data/verified-question-bank-v3-extra60.js','13200');
  load('VERIFIED_Q_V4_EXTRA','data/verified-question-bank-v4-extra100.js','13200');
  load('VERIFIED_Q_V5_EXTRA','data/verified-question-bank-v5-extra300.js','13200');
  load('VERIFIED_Q_V6_A','data/verified-question-bank-v6-a100.js','13200');
  load('VERIFIED_Q_V6_B','data/verified-question-bank-v6-b100.js','13200');
  load('VERIFIED_Q_V6_C','data/verified-question-bank-v6-c100.js','13200');
  load('VERIFIED_Q_V6_D','data/verified-question-bank-v6-d100.js','13200');
  for(let v=10;v<=19;v++) load(`VERIFIED_Q_V${v}_EXTRA`,`data/verified-question-bank-v${v}-extra100.js`,'13200');
  load('VERIFIED_Q_V20_EXTRA','data/verified-question-bank-v20-extra2000.js','13200');
  load('VERIFIED_Q_V21_EXTRA','data/verified-question-bank-v21-extra2000.js','13200');
  load('CANDIDATE_Q_V22_EXTRA','data/candidate-question-bank-v22-extra6000.js','13200');
  load('VERIFIED_Q_V23_BATCH01','data/verified-question-bank-v23-batch01-1000.js','13200');
  load('V24_AUTHORED_0001_0100','data/v24-authored-questions-0001-0100.js','13200');
  load('V24_AUTHORED_0101_0200','data/v24-authored-questions-0101-0200.js','13200');
  if(!window.CANDIDATE_Q_V22_QA){const x=new XMLHttpRequest();x.open('GET','data/candidate-question-bank-v22-qa.js?v=13200',false);x.send(null);if((x.status>=200&&x.status<300)||x.status===0)(0,eval)(x.responseText)}
  const pools=[
  ...
  pools.push(v23.map(q=>({...q,verificationStatus:'semantic-unique-reviewed'})));
  const v24a=Array.isArray(window.V24_AUTHORED_0001_0100)?window.V24_AUTHORED_0001_0100:[];
  const v24b=Array.isArray(window.V24_AUTHORED_0101_0200)?window.V24_AUTHORED_0101_0200:[];
  pools.push(v24a.map(q=>({...q,verificationStatus:'semantic-unique-reviewed'})));
  pools.push(v24b.map(q=>({...q,verificationStatus:'semantic-unique-reviewed'})));
  const expected=[90,50,60,100,300,100,100,100,100,100,100,100,100,100,100,100,100,100,100,2000,2000,6000,1000,100,100];
  ...
  if(sizeBad||combined.length!==13200||bad.length||dup.length){throw new Error(`Aktif soru havuzu QA başarısız: sizes=${sizes.join(',')}, total=${combined.length}, bad=${bad.map(q=>q.sourceId).join(',')}, dup=${dup.join(',')}`)}
  ...
  window.ARENA_ACTIVE_BANK_META={count:13200,optionCount:5,batches:sizes,status:'active',v22Qa:window.CANDIDATE_Q_V22_QA,v23QualityNote:'semantic-unique-reviewed',v24Active:200};console.info('9. Sınıf Arena aktif soru motoru:',window.ARENA_ACTIVE_BANK_META);
})();